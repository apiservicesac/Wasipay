...
export const ProductsComponent = ({ list }: { list: boolean }) => {
    ...
    return (
        <div className={`grid grid-cols-2 mt-5 md:grid-cols-3 [&.gridView]:grid-cols-1 xl:grid-cols-4 group [&.gridView]:xl:grid-cols-1 gap-x-5 ${!list && "gridView"}`} id="cardGridView">

            {(products || []).map((item: ProductItemEntity) => (
                <div key={item?.product?.id} className="card md:group-[.gridView]:flex relative">
                    <div className="relative group-[.gridView]:static p-[10px] md:p-5 group-[.gridView]:p-2">
                        <div className="group-[.gridView]:p-3 group-[.gridView]:bg-slate-100 dark:group-[.gridView]:bg-zink-600 group-[.gridView]:inline-block rounded-md">
                            <Swiper className="pagination-slider group-[.gridView]:w-24"
                                ...
                                {item?.product?.images!.map((image) => (
                                    <SwiperSlide key={image.url}><img src={image.url} className="rounded-sm" /></SwiperSlide>
                                ))}
                            ...
                            {item.in_cart ? (
                                <div className="flex items-center justify-between gap-2 w-full">
                                    <div className="inline-flex text-center input-step">
                                        ...
                                        <input type="number" className="md:w-12 text-center h-9 border-y product-quantity dark:bg-zink-700 focus:shadow-none dark:border-zink-500" value={item.quantity} min="0" max="100" readOnly />
                                ...
                                <button type="button" onClick={() => addProductCart(item)} className="w-full bg-white border-dashed text-slate-500 btn border-slate-500 hover:text-slate-500 hover:bg-slate-50 hover:border-slate-600 focus:text-slate-600 focus:bg-slate-50 focus:border-slate-600 active:text-slate-600 active:bg-slate-50 active:border-slate-600 ">
                                    <ShoppingCart className="inline-block size-4 leading-none" /> 
                                    <span className="hidden lg:inline-block align-middle">Add to Cart</span>
                                </button>
                            )}
                            <a href="#!" className="hidden md:flex text-xl w-[90px] items-center justify-center text-white transition-all duration-200 ease-linear bg-green-500 rounded size-9 hover:bg-green-500 ">
