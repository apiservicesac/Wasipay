    ...
    return (
        <div className="flex gap-2 product" id={`product${item.product!.id}`}>
            <div className="flex items-center justify-center size-12 rounded-md bg-slate-100 shrink-0 dark:bg-zink-500">
                <img src={item.product?.images![0].url} alt="" className="h-8" />
            </div>
            <div className="overflow-hidden grow">
                
                <div className="float-right">
                    <button onClick={() => removeProductCart(item.product!.id!)} className="transition-all duration-150 ease-linear text-slate-500 dark:text-zink-200 hover:text-red-500 dark:hover:text-red-500">
